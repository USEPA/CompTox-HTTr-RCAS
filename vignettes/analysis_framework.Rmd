...
initially for bioactivity (Tier 1-Active) and additionally for selectivity
...
ref <- distinct(rbind(
  select(rcas_u2os$httr.wide, dtxsid, ref_class),
  select(rcas_heparg$httr.wide, dtxsid, ref_class)
), dtxsid, ref_class)

combined_attr <- combined_sum %>%
  mutate(
    ref_class = ref$ref_class[match(dtxsid, ref$dtxsid)],
    in_class = ref_class %in% unique(combined_full$signature_label),
    class_type = case_when(
      !is.na(ref_class) & in_class ~ "RCAS\nReference",
      !is.na(ref_class) & !in_class ~ "Non-RCAS\nReference",
      TRUE ~ "Test\nChemical"
    )
  ) %>%
  group_by(dtxsid) %>%
  summarise(
    tier1meas = unique(class_type),
  ...
  mutate(
    ...
    tier2pos = case_when(
      ...
      tier1sel == "Active Only" | is.na(tier1sel) ~ NA,
...
labels_attr <- c(
  ...
  "Multiple", "Test\nChemical", "RCAS\nReference", "Non-RCAS\nReference",
  "Negative", "Active Only", "Insufficient\nOrthogonal Data"
)
fill_attr <- c(
  RColorBrewer::brewer.pal(6, "Dark2")[c(1, 3, 5, 2, 4, 6)],
  "#b1d4fc", brewer.pal(3, "Set1")[c(2, 1)], rep("grey70", 2), "navy"
)
color_attr <- c(
  rep("grey10", 9), rep("grey30", 2), "navy"
)
...
counts_attr <- combined_attr %>%
  ...
  mutate(
    ...
    count_label = case_when(
      x == "tier1meas" ~ gettextf("%d %ss", count, node),
      node %in% c("Negative", "Active Only", "Insufficient\nOrthogonal Data") ~ gettextf(
        "%d\n%s", count, node
      ),
      # x == "tier2sel" & !node %in% c("Negative", "Active Only", "Insufficient\nOrthogonal Data") ~ gettextf(
      #   "%d Candidates", count
      # ),
...
for (sig in unique(combined_sum$source_signature)) {
  assoc <- combined_sum %>%
    filter(source_signature == sig & n_toxcast_meas >= 2) %>%
...
plot_or <- rbind(
  mutate(or_positive, comparison = "All Active Chemicals"),
  mutate(or_selective, comparison = "Active + Selective Chemicals")
) %>%
  mutate(
    comparison = factor(
      comparison, levels = c("All Active Chemicals", "Active + Selective Chemicals")
    ),
...
ggsave(
  paste0(dir_home, "output/Fig2.tiff"), plot_fig2,
  width = 2000, height = 2800, units = "px", bg = "white",
...
to those with a positive or selective Tier 1 outcome via Wilcoxon rank-sum
tests.

```{r compare-mismatch-outcome}
# get summary of true/false negatives
combined_sum %>%
  mutate(
    outcome_tier2 = case_when(
      outcome_tier2 != "negative" & n_toxcast_meas < 2 ~ "negative",
      TRUE ~ outcome_tier2
    )
  ) %>%
  count(source_signature, outcome_tier1, outcome_tier2) %>%
  ggplot(aes(outcome_tier2, n, fill = outcome_tier1, label = n)) +
  geom_col(position = position_fill()) +
  geom_text(position = position_fill(vjust = 0.5)) +
  facet_wrap(~source_signature, nrow = 1) +
  labs(
    x = "Tier 2 Outcome",
    y = "Proportion of Unique Chemicals",
    fill = "Tier 1 Outcome"
  ) +
  theme_bw() +
  th
  # filter(outcome_tier1 == "negative") %>%
  # mutate(negative_tier1 = ifelse(outcome_tier2 == "negative", "TN", "FN")) %>%
  # group_by(source_signature, negative_tier1) %>%
  ...
  mutate(
    rs = purrr::map(
      data, ~ pairwise.wilcox.test(
        .x$modl_acc, .x$tier1_positive,  p.adjust.method = "BH"
      )
    ),
...
contr_pos <- mismatch_contr %>%
  filter(!tier2_selective & group2 == "FALSE") %>%
...
plot_mismatch_pos <- combined_sum %>%
  ...
  mutate(
    outcome_tier1_label = case_when(
      outcome_tier1 %in% c("positive", "selective") ~ "Active",
      # outcome_tier1 == "selective" ~ "Active + Selective",
      outcome_tier1 == "negative" ~ "Negative"
    ),
    outcome_tier1_label = factor(
      outcome_tier1_label,
      # levels = c("Negative", "Active Only", "Active + Selective")
      levels = c("Negative", "Active")
    )
  ) %>%
  ...
  labs(
    x = expression(log[10](POD[ortho])), y = "RCAS", color = "Tier 1 Activity Result",
    title = "Tier 2 Active Only"
  ) +
  theme_bw() +
  th +
...
plot_mismatch_sel <- combined_sum %>%
  ...
  geom_signif(
    ...
    y_position = c(2.8, 2.8, 2.8, 2.8),
    # xmin = c(0.7, 0.7, 1.7, 1.7, 2.8, 3.7, 3.7),
    xmin = c(0.7, 1.8, 2.8, 3.8),
...
plot_mismatch_all <- cowplot::plot_grid(
  ...
  cowplot::plot_grid(
    NULL, cowplot::get_plot_component(plot_mismatch_sel, "guide-box-bottom"),
    rel_widths = c(0.2, 1)
  ),
...
ggsave(
  paste0(dir_home, "output/Fig3.tiff"), plot_mismatch_all,
  width = 2250, height = 1440, units = "px", bg = "white",
  device = "tiff", compression = "lzw"
)
plot_mismatch_all
```
  ...
  if (tier2) {
    combined_filt <- combined_sum %>%
      filter(
        ...
        specific_crit_2 == "median_bioactive"
      ) %>%
      group_by(source_signature, outcome_tier2)
  } else {
    combined_filt <- combined_sum %>%
      filter(
        outcome_tier1 != "negative" &
        specific_crit_1 == "median_bioactive"
      ) %>%
  ...
  if (tier2) {
    cor_rcas <- mutate(
      ...
      outcome_label = case_when(
        outcome_tier2 == "selective" ~ "Active + Selective",
        TRUE ~ "Active Only"
      )
    )
  } else {
...
plotPods <- function(combined_sum, cor_rcas, col_compare, label_compare, tier2 = FALSE) {
  ...
  plot_pod <- combined_filt %>%
    ...
    geom_text(
      data = filter(cor_rcas, outcome_label == "Active Only"),
      aes(label = cor_label),
      x = -1.8, y = 2.5, show.legend = FALSE, family = "Helvetica"
    ) +
    geom_text(
      data = filter(cor_rcas, outcome_label == "Active + Selective"),
      aes(label = cor_label),
    ...
    theme_bw() +
    th +
    theme(legend.position = "bottom", strip.text.x = element_text(size = 7.5)) +
    guides(label = "none")
...
plot_tier2 <- plotPods(combined_sum, cor_tier2, "modl_acc", "ortho", tier2 = TRUE)
...
ggsave(
  paste0(dir_home, "output/Fig4.tiff"), plot_pod_all,
  width = 2250, height = 1767, units = "px", bg = "white",
  device = "tiff", compression = "lzw"
)
...
ggsave(
  paste0(dir_home, "output/FigS7.png"), plot_bpac2,
  ...
  combined_proc <- combined_filt %>%
    ...
    mutate(
      ...
      name_label = case_when(
        tolower(gsub(",", "-", name)) %in% struct ~ name,# |
...
plotCandidates <- function(combined_proc) {
  ...
  plot_candidates <- ggplot() +
    geom_line(
      ...
      alpha = 0.8, position = position_dodge(width = .7), show.legend = TRUE
    ) +
    geom_point(
      data = combined_proc,
      aes(
    ...
    # scale_y_continuous(limits = c(-3.5, 2.1)) +
    scale_color_manual(
      values = c(
        "grey50",
    ...
    scale_shape_manual(
      values = c(4, 16, 21),
      labels = expression(POD[RCAS], POD[ortho], POD[`NS2`])
    ) +
    # facet_wrap(~source_signature, nrow = 1) +
    labs(
      x = "", y = expression(log[10](POD)),
...
# specify chemical labels by those named in manuscript (in lieu of pngs)
struct_ms <- c(
  "menadione", "2-aminoanthraquinone", "2-naphthalenol",
  "1-naphthol", "1-4-benzoquinone",
  "dichlone", "1-4-dihydroxyanthracene-9-10-dione",
  "3-hydroxyfluorene", "fluorometholone", "medroxyprogesterone acetate",
  "budesonide", "trilostane", "triflumizole", "fusilazole", "imazalil",
  "prochloraz", "etoxazole", "aldrin", "endrin", "lindane"
)
```
...
ggsave(
  paste0(dir_home, "output/Fig5_alt.tiff"), plots_candidate_struct,
  width = 2250, height = 2500, units = "px", bg = "white",
  device = "tiff", compression = "lzw"
)
...
```{r export-or}
# compile odds ratios and chemical totals for export
totals_positive <- assoc_positive$tbls %>%
  select(source_signature, tier1_positive, tier2_positive, .observed) %>%
  mutate(
    across(c(tier1_positive, tier2_positive), as.logical),
    tier1_positive = case_when(tier1_positive ~ "pass", !tier1_positive ~ "fail"),
    tier2_positive = case_when(tier2_positive ~ "pass", !tier2_positive ~ "fail"),
    comparison = "All Active Chemicals"
  ) %>%
  pivot_wider(
    names_from = c(tier1_positive, tier2_positive),
    values_from = .observed
  ) %>%
  rename(
    total_neither = fail_fail,
    total_tier1_only = pass_fail,
    total_tier2_only = fail_pass,
    total_both = pass_pass
  )

totals_selective <- assoc_selective$tbls %>%
  select(source_signature, tier1_selective, tier2_selective, .observed) %>%
  mutate(
    across(c(tier1_selective, tier2_selective), as.logical),
    tier1_selective = case_when(tier1_selective ~ "pass", !tier1_selective ~ "fail"),
    tier2_selective = case_when(tier2_selective ~ "pass", !tier2_selective ~ "fail"),
    comparison = "Active + Selective Chemicals"
  ) %>%
  pivot_wider(
    names_from = c(tier1_selective, tier2_selective),
    values_from = .observed
  ) %>%
  rename(
    total_neither = fail_fail,
...
totals_combined <- rbind(totals_positive, totals_selective)
or_combined <- rbind(
  mutate(or_positive, comparison = "All Active Chemicals"),
  mutate(or_selective, comparison = "Active + Selective Chemicals")
)
totals_combined <- left_join(
  totals_combined, or_combined, by = c("source_signature", "comparison")
) %>%
  rename(rcas = source_signature) %>%
  select(-c(method, alternative))
openxlsx::write.xlsx(totals_combined, "output/TableS3.xlsx")
```
