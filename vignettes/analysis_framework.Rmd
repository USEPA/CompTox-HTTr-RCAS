...
```{r visualize-outcome-associations, warning=FALSE, fig.width=6, fig.height=3}
...
```{r test-mismatch-contrasts, fig.width=6, fig.height=3.4}
...
plot_mismatch_pos <- combined_sum %>%
  ...
  ggplot(aes(modl_acc, source_signature, color = outcome_tier1)) +
  ...
  labs(
    x = expression(log[10](POD[Tier~2])), y = "RCAS", color = "Tier 1 Outcome",
...
derived BMDs are compared to 5th percentile overall HTTr BMDs (aka Biological
Phenotype Altering Concentrations, BPACs)($POD_{BPAC}$) and ToxCast orthogonal
BMDs ($POD_{Tier 2}$).
...
cor_bpac %>%
  select(-c(n_chems, alternative, cor_label, outcome_label)) %>%
  mutate(across(where(is.numeric), ~signif(.x, 2))) %>%
  DT::datatable()
...
plotPods <- function(combined_sum, cor_rcas, col_compare, label_compare, tier2 = FALSE) {
  ...
  plot_pod <- combined_filt %>%
    ...
    geom_text(
      ...
      aes(label = cor_label),
      x = -1.8, y = 2.5, show.legend = FALSE
    ) +
    geom_text(
      data = filter(cor_rcas, outcome_label == "Positive + Selective"),
      aes(label = cor_label),
      x = -1.8, y = 1.8, show.legend = FALSE
    ) +
...
plot_bpac <- plotPods(combined_sum, cor_bpac, "bmd_log_5", "BPAC")
plot_tier2 <- plotPods(combined_sum, cor_tier2, "modl_acc", "Tier 2", tier2 = TRUE)
...
```{r pod-comparison-bpac}
# calculate correlation between RCAS BMDs and HTTr BPACs
cor_bpac <- combined_sum %>%
  filter(outcome_tier1 != "negative") %>%
  distinct(dtxsid, source_signature, .keep_all = TRUE) %>%
  group_by(source_signature, outcome_tier1) %>%
  select(dtxsid, bmd_log, bmd_log_5) %>%
  nest() %>%
  mutate(n_chems = unlist(purrr::map(data, nrow))) %>%
  filter(n_chems >= 3) %>%
  mutate(
    cor_obj = purrr::map(data, ~ cor.test(.x$bmd_log, .x$bmd_log_5)),
    cor_df = purrr::map(cor_obj, broom::tidy)
  ) %>%
  unnest(cor_df) %>%
  mutate(
...
# plot RCAS BMDs against HTTr BPACs
plot_bpac <- combined_sum %>%
  filter(outcome_tier1 != "negative") %>%
  distinct(dtxsid, source_signature, .keep_all = TRUE) %>%
  mutate(
    tier1_label = case_when(
  ...
  ggplot(aes(bmd_log, bmd_log_5, color = tier1_label)) +
  geom_point(na.rm = TRUE, alpha = 0.7) +
  geom_abline(
    slope = 1, intercept = c(-0.5, 0.5), linetype = "dashed", color = "grey50"
  ) +
  geom_abline(slope = 1, intercept = 0, color = "grey50") +
  geom_text(
    data = filter(cor_bpac, tier1_label == "Positive"),
    aes(label = cor_label),
    x = -1.8, y = 2.5, show.legend = FALSE
  ) +
  geom_text(
    data = filter(cor_bpac, tier1_label == "Selective"),
    aes(label = cor_label),
    x = -1.8, y = 1.8, show.legend = FALSE
  ) +
  facet_wrap(~source_signature, nrow = 1) +
  labs(
    ...
    y = expression(log[10](POD[BPAC])),
    color = "Tier 1 Outcome"
  ) +
  scale_color_manual(values = scales::hue_pal()(3)[c(3, 2)]) +
  scale_x_continuous(limits = c(-3, 3)) +
  scale_y_continuous(limits = c(-3, 3)) +
...
criteria for comparison of potency estimates across targeted assays.

```{r inspect-candidates, class.source = 'fold-hide'}
...
plotCandidates <- function(combined_proc) {
  ...
  plot_candidates <- ggplot() +
    ...
    scale_shape_manual(
      values = c(4, 16, 21),
      labels = expression(POD[RCAS], POD[Orthogonal], POD[ToxCast])
    ) +
    # facet_wrap(~source_signature, nrow = 1) +
    labs(
      x = "", y = "log10(POD)",
...
```{r plot-structures, class.source = 'fold-hide'}
...
struct_plot[["AHR"]] <- cowplot::plot_grid(
  structs[["1-4-diaminoanthraquinone"]], structs[["9-phenanthrol"]],
  structs[["2-aminoanthraquinone"]],
  labels = c("1,4-Diaminoanthraquinone", "9-Phenanthrol", "2-Aminoanthraquinone"),
...
struct_plot[["RAR"]] <- cowplot::plot_grid(
  ...
  labels = c("Imazalil", "Prochloraz", "Aldrin", "Dieldrin"),
  label_fontface = "plain", label_size = 10, label_fontfamily = "Helvetica",
  label_x = 0.5, label_y = 1, hjust = 0.5
)
```

```{r plot-candidates, fig.width=5.3, fig.height=8}
...
ggsave(
  paste0(dir_home, "output/Fig6.png"), plots_candidate_struct,
