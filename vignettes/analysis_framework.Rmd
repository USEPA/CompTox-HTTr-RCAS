...
combined_attr <- combined_sum %>%
  ...
  mutate(
    ...
    tier2pos = case_when(
      tier1sel %in% c("Multiple", unique(combined_sum$source_signature)) & lengths(tier2meas) == 0 ~ "No Orthogonal\nEndpoints",
    ...
    tier2sel = case_when(
      tier2pos == "Negative" | tier2pos == "No Orthogonal\nEndpoints" | is.na(tier2pos) ~ NA,
...
labels_attr <- c(
  ...
  "Multiple", "Profiled", "Negative", "Positive Only", "No Orthogonal\nEndpoints"
)
fill_attr <- c(
...
counts_attr <- combined_attr %>%
  ...
  mutate(
    ...
    count_label = case_when(
      ...
      node %in% c("Negative", "Positive Only", "No Orthogonal\nEndpoints") ~ gettextf(
        "%d\n%s", count, node
      ),
      # x == "tier2sel" & !node %in% c("Negative", "Positive Only", "No Orthogonal\nEndpoints") ~ gettextf(
      #   "%d Candidates", count
      # ),
...
plot_attr <- combined_attr %>%
  ...
  geom_segment(
    aes(x = 4.75, y = -100, xend = 5, yend = -100),
  ...
  geom_text(
    aes(x = 5, y = -100, label = "Prioritized for\nTargeted Assays"),
    color = "navy",
    size = 2.5,
    hjust = 0,
    nudge_x = 0.1,
...
plot_or <- rbind(
  mutate(or_positive, comparison = "Positive Only"),
  mutate(or_selective, comparison = "Positive + Selective")
) %>%
  mutate(
    comparison = factor(
      comparison, levels = c("Positive Only", "Positive + Selective")
    ),
    p_label = ifelse(
      p.value < 1E-3,
      paste0("p = ", formatC(p.value, format = "e", digits = 2)),
      paste0("p = ", (as.character(signif(p.value, 2))))
    )
  ) %>%
  ggplot() +
  ...
  # geom_text(
  #   aes(y = source_signature, x = conf.high + 1, label = p_label),
  #   color = "grey30",
  #   angle = 270,
  #   size = 2.5, 
  # ) +
...
ggsave(
  ...
  width = 1000, height = 1200, units = "px", scale = 2, bg = "white"
)
```
...
plot_bpac <- plotPods(combined_sum, cor_bpac, "bmd_log_5", "BPAC-HTTr")
...
plotCandidates <- function(combined_proc) {
  ...
  plot_candidates <- ggplot() +
    ...
    scale_shape_manual(
      values = c(4, 16, 21),
      labels = expression(POD[RCAS], POD[Orthogonal], POD[`NS-ToxCast`])
    ) +
    # facet_wrap(~source_signature, nrow = 1) +
    labs(
      x = "", y = "log10(POD)",
